...
function Home({ search, scrollHandel, shandler }: searchprop) {
	...
	return (
		<>
			...
			{spage.length > 1 ? (
				<nav
					style={{ backgroundColor: "unset", marginTop: "90px", justifyContent: "center" }}
					aria-label="..."
				>
					<ul style={{ boxShadow: "0 0 20px -10px black" }} className="pagination">
						<li
							onClick={() => {
								if (cpage > 1) {
						...
						<li
							...
							className={`page-item ${cpage === mpage.length ? "disabled" : null}`}
						>
							<a className="page-link" href="#">
								Next
							</a>
